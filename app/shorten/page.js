              ...
              <tbody>
                {allLinks.map((link) => (
                  <tr key={link._id} className="border-b border-violet-50 hover:bg-rose-50/30 transition-colors">
                    ...
                    <td className="p-4 text-sm font-semibold text-violet-600"><Link href={link.shorturl} target='_blank'>{link.shorturl}</Link><div className='text-end'><button onClick={() => copyLink(`https://${link.url}`)} className="px-3 py-1 bg-gray-200 rounded hover:bg-gray-300">Copy</button></div></td>
